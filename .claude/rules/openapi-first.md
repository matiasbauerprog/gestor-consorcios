...
- Los path parameters usan el nombre completo del recurso: `{expensa_id}`, `{peticion_id}`, `{trabajo_id}`, `{amenity_id}`, `{reserva_id}`, `{comprobante_id}`, `{departamento_id}`, `{usuario_id}` — no `{id}` genérico.
